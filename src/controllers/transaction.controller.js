...
async function createTransaction(req,res){
    ...
    if(!fromUserAccount || !toUserAccount){
        return res.status(400).json({
            message : "fromAccount and toAccount must be associated with an existing account",
            status : "failed"
        })
    }
     ...
     */
    await emailService.sendTransactionEmail(req.user.email, req.user.name, amount,
        toAccount
    );
